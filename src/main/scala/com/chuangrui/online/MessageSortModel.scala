...
object MessageSortModel {

  def main(args: Array[String]): Unit = {

    val hdfsUrl = "hdfs://cr"

    val conf = new SparkConf().setAppName("MessageSortModel")
    ...
    val inputRDD = sc.textFile(hdfsUrl+"/data/sort/*.txt").repartition(12)
  ...
  //  wordVector.foreach(println)
    ...
    //保存模型
    val ModelPath = "/model/"+timeStr.replace("-","/")+"/"
