...
import com.chuangrui.utils.ChuangFuConnectionUtils
...
object ChuangFuMessageSortModel {

  def main(args: Array[String]): Unit = {

    val hdfsUrl = "hdfs://cr"

    val conf = new SparkConf().setAppName("ChuangFuMessageSortModel")
    ...
    //获取所有分类
    val signMap:util.HashMap[String,Integer] = ChuangFuConnectionUtils.getSign()
    System.out.println("signMap signMap signMap signMap signMap:"+signMap.size())

    val inputRDD = sc.textFile(hdfsUrl+"/data/chuangfu/dataset/*.txt").repartition(12)
  ...
  //  wordVector.foreach(println)
    ChuangFuConnectionUtils.saveWord(wordVector,timeStr)
    ...
    //保存模型
    val ModelPath = "/cfmodel/"+timeStr.replace("-","/")+"/"
