...
object ReplySortModel {

  def main(args: Array[String]): Unit = {

    val hdfsUrl = "hdfs://cr"

    val conf = new SparkConf().setAppName("ReplySortModel")
   ...
   val zangci:util.List[String] = ConnectionUtils.getZangCi()
    System.out.println("zangci list:"+zangci.size())
    val zcList = sc.broadcast(zangci)


    val inputRDD = sc.textFile(hdfsUrl+"/data/reply_datasort/*.txt")
    inputRDD.persist(StorageLevel.MEMORY_ONLY_SER)

    val wordRDD = inputRDD.flatMap(line =>{
      val ss = line.split(",")
      var wordList = PaoDingCut.cutString(ss(1))
  ...
  //  wordVector.foreach(println)
    ConnectionUtils.saveReplyWord(wordVector,timeStr)
    ...
    val parsedData = inputRDD.map(line => {
      ...
      LabeledPoint(ss(0).toDouble, Vectors.dense(array))
    })
    ...
    //保存模型
    val ModelPath = "/replyModel/"+timeStr.replace("-","/")+"/"
