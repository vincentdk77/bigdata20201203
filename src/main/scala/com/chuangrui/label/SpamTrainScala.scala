...
import com.chuangrui.schema.TempSchema
...
object SpamTrainScala {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //获取所有分类
    val signMap:util.HashMap[String,Integer] = ConnectionUtils.getSign()
    System.out.println("signMap signMap signMap signMap signMap:"+signMap.size())

    val inputRDD = sc.textFile("hdfs://node1:9000/data/sort/*.txt").repartition(12)
    ...
    pairRDD.map(tuple=>{(tuple._2,tuple._1)}).sortByKey(false).saveAsTextFile("hdfs://node1:9000/data/pairRDD1")
   ...
   val handleRDD =  pairRDD.map(tuple => {
    ...
    //handleRDD.saveAsTextFile("hdfs://node1:9000/data/handleRDD1")

   val wordVector:Array[String] = handleRDD.map(tuple=>{tuple._2}).take(wordNumber)
  //  wordVector.foreach(println)
    ConnectionUtils.saveWord(wordVector,timeStr)
    ...
    //保存模型
    val ModelPath = "/naive_bayes_model/"+timeStr.replace("-","/")+"/"
