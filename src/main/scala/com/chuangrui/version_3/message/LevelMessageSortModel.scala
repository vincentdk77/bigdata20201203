...
object LevelMessageSortModel {

  def main(args: Array[String]): Unit = {

    val hdfsUrl = "hdfs://cr"

    val conf = new SparkConf().setAppName("LevelMessageSortModel")
    val sc = new SparkContext(conf)
    val sqlContext = new SQLContext(sc)

    var timeStr = args.apply(0)
    var wordNumber = args.apply(1).toInt
    val level = args.apply(2).toInt
    val signCode = args.apply(3)

    System.out.println("level:"+level+" signCode:"+signCode+" wordNumber:"+wordNumber);
    var levelStr = "first";
    var path = "/data/dataset/message/first/*.txt"
    var savePath = "/model/version3/first/"
    if(level == 2) {
      levelStr = "second"
      path = "/data/dataset/message/"+levelStr+"/"+signCode+"/*.txt"
      savePath = "/model/version3/"+levelStr+"/"+signCode+"/"
    } else if(level == 3) {
      levelStr = "third"
      path = "/data/dataset/message/"+levelStr+"/"+signCode+"/*.txt"
      savePath = "/model/version3/"+levelStr+"/"+signCode+"/"
    ...
    val signId = ConnectionSignMgrUtils.getSignIdByCode(signCode)
    System.out.println("signId:"+signId+" level:"+level);
    //获取该级所有标签
    val signMap:util.HashMap[String,Integer] = ConnectionSignMgrUtils.getSign(level,signId);
    System.out.println("signMap signMap signMap signMap signMap:"+signMap.size())

    val inputRDD = sc.textFile(hdfsUrl+path).repartition(12).filter(line=>{
  ...
  //  wordVector.foreach(println)
    ConnectionSignMgrUtils.saveWord(wordVector,timeStr,signCode,level)
    ...
    //保存模型
    val ModelPath = savePath+timeStr.replace("-","/")+"/"
