...
import com.chuangrui.version_3.utils.{ConnectionSignMgrUtils, HbaseUtilsVersion3}
...
//  var zcList:Broadcast[util.List[String]] = null
  def main(args: Array[String]): Unit = {
    val hdfs = "hdfs://cr"
    val conf = new SparkConf().setAppName("SaveSecondToHbase")
  ...
  val secondYxModel = sc.broadcast(secondYxMsgModel)
    ...
    while (startTime <= endTime) {
      ...
      val tzRDD: RDD[(String, JSONObject)] = tzinputRDD.map(line => {
        var json: JSONObject = JSONUtils.getVersion3Json(line)
      ...
      for (key <- tongzhiSignMap.keySet()) {
        ...
        val signCode:String = tongzhiSignMap.get(key)
        val signRDD:RDD[JSONObject] = tzRDD.filter(tuple => {
        ...
        signRDD.persist(StorageLevel.MEMORY_ONLY)
        //处理黑名单
        handleBlack(signCode,signRDD,sc,sf,startTime,partition,ccMap,blackMap)
        //保存频次
        handleFrequency(signCode,signRDD,sc,sf,startTime,partition)
        //客户评分
        handleCustomer(signCode,signRDD,sc,sf,startTime,partition)
        //回复评分
        if(sf.format(startTime).compareTo("2018-08-11") > 0) {
          handleReply(signCode,signRDD,sc,sf,startTime,partition,replyArray,replyModel,zcList)
        }
        signRDD.unpersist()
      }
      tzRDD.unpersist()
 ...
 //   resultRDD.saveAsTextFile("hdfs://cr/data/message/msg-reply/"+sf.format(startTime)+"/")
    ...
    val strRDD =resultRDD.map(json =>{
      ...
      (index,ss.apply(0)+"@"+json.toJSONString)
    }).groupByKey().map(tuple=>{
